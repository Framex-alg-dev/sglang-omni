...
#
# Checks path safety, dependency manifest fingerprint (when recorded), import
# probe, and exact == pins. Missing .deps-hash is allowed when the venv itself
# matches the dependency manifests (e.g. prior setup installed packages but
# failed a post-install gate).
...
if [ -f "${DEPS_HASH_FILE}" ]; then
  ...
  if [ "${STORED_HASH}" != "${DEPS_HASH}" ]; then
    ...
    echo "pyproject.toml or uv.lock changed; full environment rebuild required" >&2
    exit 1
  fi
else
  echo "Note: ${DEPS_HASH_FILE} missing; validating installed venv against dependency manifests"
fi
