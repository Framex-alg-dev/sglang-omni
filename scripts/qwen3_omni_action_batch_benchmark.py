#!/usr/bin/env python3
"""Benchmark protocol-v1 realtime action scoring at several micro-batch sizes."""
...
DEFAULT_CATALOG = "sglang_omni/assets/character_action_global_catalog.json"
...
def build_allowed_candidates(path: str) -> tuple[list[dict[str, Any]], int, int]:
    ...
    for category in categories:
        ...
        if not isinstance(children, list) or not children:
            raise ValueError(f"invalid global category: {category!r}")
...
async def run_once(
    ...
    async with websockets.connect(base_url, open_timeout=timeout_s, close_timeout=timeout_s, max_size=64 * 1024 * 1024) as ws:
        await ws.send(json.dumps({
            ...
            "outputs": ["action"],
            "locale": "zh-CN",
            "action": {"allowed_candidates": catalog},
            "input_audio": {
                "format": "pcm16le",
        ...
        text_ack, events = await recv_until(ws, {"input.text.ack", "error"}, timeout_s)
        result["event_types"].extend(events)
        if text_ack.get("type") != "input.text.ack":
            result["error"] = text_ack
            return result
        ...
        await ws.send(json.dumps({"type": "session.close", "reason": "benchmark_complete"}))
    return result
