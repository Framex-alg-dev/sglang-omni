...
ACTION_INTENT_POLICY = (
    ...
    "动作请求由语义目标决定，不由命令句形式决定。疑问句形式本身既不能证明用户在要求"
    "执行动作，也不能证明用户只是在询问能力；必须判断用户期望本轮得到的是能力信息，还是"
    "立即发生的外部可观察行为。“你可以唱歌吗”“你会唱歌吗”等没有要求立即表演、也没有"
    "指定交付内容的表达，属于能力信息询问；本轮产生非空回复时应选择语言表达伴随类别。"
    "“给我唱一首”“现在唱一段吧”“唱给我听”等明确要求立即表演或向用户交付表演的表达，"
    "属于动作请求；若没有具体动作能够完成，再由子动作阶段判断不支持。“你可以挥挥手吗”"
    "虽然使用问句，但期望结果就是当前角色立即产生挥手行为，仍属于动作请求。“请……”"
    "“……吧”“帮我……”等委婉形式也必须按同一语义边界判断。以上示例说明能力信息与"
    "立即执行目标的边界，不是关键词匹配规则。"
...
ACTION_INTENT_POLICY_EN = (
    ...
    "Interrogative form alone proves neither an execution request nor a mere capability question. "
    "Determine whether the user expects capability information or an externally observable behavior "
    "to occur now. Bare questions such as 'can you sing?' or 'do you know how to sing?' that do not "
    "request an immediate performance or specify content to deliver ask for capability information; "
    "use the reply-accompaniment category when a non-empty reply is produced. Requests such as "
    "'sing me a song', 'sing something now', or 'sing for me' ask for an immediate performance and "
    "are action requests; let the child-action stage decide unsupported when no concrete action can "
    "fulfill them. 'Can you wave?' still requests an action because its expected result is the "
    "character immediately waving. Apply the same semantic boundary to polite forms such as "
    "'please ...', '... for me', and 'why don't you ...?'. These examples illustrate capability "
    "information versus immediate execution; they are not keyword-matching rules. Asking whether "
    "the character can currently see, hear, or otherwise perceive the user, "
