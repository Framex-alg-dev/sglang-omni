    ...
    def _pure_action_reply_validation_system_prompt(self) -> str:
        return self._prompt(
            zh=(
                ...
                "V0=合法短回应：只表达接受、配合或自然互动；保持原请求的执行者、对象、目标、"
                "受益者和人称关系；不描述具体动作，也不增加独立话题。"
                "V1=动作语义改变：复述、描述、承诺、重命名或替换具体动作；改变动作执行者、"
                "动作对象、目标、受益者或身体部位归属；把要求当前角色执行的动作改成让用户"
                "执行、让用户选择，或者改成另一项动作。"
                "V2=状态或情境编造：无依据地声称角色当前具有某种情绪、感受、身体状态、环境"
                "状态或经历。"
                "V3=无关内容：延续其他话题，增加与当前请求无关的问题、建议、介绍或不必要展开；"
                "如果内容已经属于 V1，则选择 V1，不再选择 V3。"
                "V4=身份、拒绝或不可朗读格式：谈论数字人、模型、实体能力或内部动作系统，拒绝"
                "身体动作，或者包含 Markdown、舞台说明、标签、解释、换行等非自然口语格式。"
                "人设语气本身不是违规，但人设不能成为编造当前状态、改变动作语义或转移话题的"
                "理由。任何一项违规存在时都不得选择 V0；若同时存在多项违规，按 V4、V1、V2、"
                "V3 的顺序选择。以下示例只用于判定违规类型，不是回复模板："
                "‘你挑个舞吧’→V1；‘我帮你摸摸’→V1；‘你也挥挥手吧’→V1；"
            ...
            en=(
                ...
                "Choose exactly one result. V0=valid brief response: expresses only acceptance, "
                "cooperation, or natural interaction; preserves the original actor, object, "
                "target, beneficiary, and pronoun relationships; does not narrate a concrete "
                "action or add an independent topic. V1=changed action semantics: repeats, "
                "narrates, promises, renames, or substitutes the concrete action; changes the "
                "actor, object, target, beneficiary, or ownership of a body part; turns an action "
                "for the current character to perform into an instruction or choice for the user, "
                "or substitutes another action. V2=invented state or situation: asserts an "
                "unsupported current emotion, feeling, body state, environment, or experience. "
                "V3=unrelated content: continues another topic or adds an unrelated question, "
                "suggestion, introduction, or unnecessary expansion. If V1 applies, choose V1 "
                "rather than V3. V4=identity, refusal, or non-spoken format: discusses digital "
                "characters, models, embodiment, or internal action systems; refuses a physical "
                "action; or contains Markdown, stage directions, labels, explanation, line breaks, "
                "or other non-spoken formatting. Persona tone is allowed, but cannot justify "
                "inventing state, changing action semantics, or changing the topic. Never choose "
                "V0 when any violation applies. If multiple violations apply, use this precedence: "
                "V4, V1, V2, then V3. The following examples classify violations and are not "
                "response templates: 'You pick a dance' -> V1; 'I will touch it for you' -> V1; "
                "'You wave too' -> V1; 'I feel shy now' -> V2; 'Hi, how was your day?' -> V3; "
