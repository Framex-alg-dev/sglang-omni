...
def request(**kwargs):
    return ActionSuffixScoreRequest(
        ...
        candidates=[ActionScoreCandidate(candidate_id="001", suffix="001")],
...
def test_repeated_identity_does_not_claim_kv_hit():
    ...
    req = request()
    first = observer.submitted(req, ["02", "01"], "zh-CN")
    assert first["selected_category_ids"] == ["02", "01"]
    ...
    second = observer.submitted(request(request_id="request-2"), ["02", "01"], "zh-CN")
...
def test_namespace_instruction_and_candidate_order_identities():
    ...
    for other in [request(prefix_cache_namespace="other"), request(session_instruction="different")]:
        assert observer.submitted(other, ["02", "01"], "zh-CN")["prefix_identity"] != first["prefix_identity"]
    reordered = observer.submitted(req, ["01", "02"], "zh-CN")
...
def test_render_cache_preserves_order_and_invalidates_changed_descriptions():
    ...
    class Prompts(ActionPromptComponent):
        ...
        def _action_prompt(self, *, zh, en):
            return en

    body = SessionActionCandidate(
        candidate_id="001", action_id="wave", source_label="wave",
        short_definition="Wave your hand", execution_binding={}, category_id="01",
    )
    cat = SessionActionCategory(
        category_id="01", source_label="greeting", short_definition="greeting",
        category_path=("greeting",), children=(body,),
    )
    other = replace(body, candidate_id="002", short_definition="Nod your head")
    prompt = Prompts()
